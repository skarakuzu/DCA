...
void DcaLoop<ParametersType, DcaDataType, MCIntegratorType>::execute() {
  if (concurrency.id() == concurrency.first()) std::cout << "\n Starting " << parameters.get_dca_iterations() << " DCA iterations.\n";

   // Store initial Sigma and use this in every iteration if ...
